...
function makeInvisible(e) {
  ...
  document.getElementById(
    "sidebar-greeting"
  ).textContent = `Hello, ${currentUsername}!`;
  e.preventDefault();
...
function addMessage(text, user, isUser = false) {
  ...
  messageElement.textContent = isUser
...
let coinsToUSD = 0.0047;
let USDToCoins = 209.1463;
const tiktokCut = 0.57;

convertCurrencyBtn.addEventListener("click", function () {
  let userConversionInput = parseFloat(
  ...
  if (conversionSelectValue == "USD-to-coins") {
    ...
    resultText = `
                <p><strong>${userConversionInput.toFixed(
                  2
                )} EUR</strong> ≈ <strong>${result.toFixed(
      3
    )} Coins</strong></p>
                <p>💰 TikTok Takes: <strong>${tiktokTakes.toFixed(
                  3
                )} Coins</strong></p>
                <p>🎉 Creator Gets: <strong>${creatorGets.toFixed(
                  3
                )} Coins</strong></p>
            `;
  } else {
    ...
    <p><strong>${userConversionInput.toFixed(
      3
    )} Coins</strong> ≈ <strong>${result.toFixed(3)} EUR</strong></p>
    <p>💰 TikTok Takes: <strong>${tiktokTakes.toFixed(3)} EUR</strong></p>
    <p>🎉 Creator Gets: <strong>${creatorGets.toFixed(3)} EUR</strong></p>
`;
  }
...
const observer = new IntersectionObserver(
  ...
  { threshold: 0.5 }
); // 50% of the iframe must be in view
...
function startTimer(duration) {
  ...
  timerInterval = setInterval(() => {
    ...
    timerOverlay.textContent = `⏳${minutes}:${
...
function showSessionEndPanel() {
  ...
  stopVimeoVideos();
  stopFloatingAlerts();
  // scroll feed to top, then hide window
  setTimeout(() => {
    resetFeedToTop();
  }, 50);
